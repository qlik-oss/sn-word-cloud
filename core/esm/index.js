/*
* sn-word-cloud v0.0.1
* Copyright (c) 2021 QLIK
* Released under the MIT license.
*/

import { useElement, useLayout, useSelections, usePromise } from '@nebula.js/stardust';
...
var COLOR_SCALES = Object.freeze({
  ...
  (function (root) {
    ...
    function Random(engine) {
      ...
      if (engine == null) {
        ...
        throw new TypeError("Expected engine to be a function, got " + _typeof(engine));
      }
    ...
    Random.date = function (start, end) {
      if (!(start instanceof Date)) {
        throw new TypeError("Expected start to be a Date, got " + _typeof(start));
      } else if (!(end instanceof Date)) {
        throw new TypeError("Expected end to be a Date, got " + _typeof(end));
      }
...
function draw(words, layout, element, selectValuesFunc, scaleColor, id, width, height, resolve) {
  ...
  var maxWordValue = data.reduce(function (accum, word) {
    return Math.max(accum, word.value);
  }, 0);
  var fill = null;

  if (layout.customRange) {
    fill = d3.scaleLinear().interpolate(d3.interpolateHcl).domain([1, maxWordValue]).range([layout.colorTo.color, layout.colorFrom.color]);
  } else {
    fill = d3.scaleLinear().interpolate(d3.interpolateHcl).domain(_toConsumableArray(Array(scaleColor.length).keys()).map(function (i) {
      return (i + 1) / scaleColor.length * maxWordValue;
    })).range(scaleColor);
  }

  var svg = d3.select("#".concat(id)).append("svg").attr("width", width).attr("height", height).attr("class", "wordcloud").append("g").attr("transform", "translate(" + [width / 2, height / 2] + ")");
  svg.selectAll("text").data(data).enter().append("text").style("fill", function (d) {
    return fill(d.value);
  }).attr("class", "selectable").attr("data-value", function (d) {
    return d.elemNumber;
  ...
  element.find(".selectable").on("qv-activate", function (_ref) {
    var target = _ref.target;
    var valueFromHyperCube = layout.qHyperCube.qDataPages[0].qMatrix.find(function (d) {
      return d[0].qText === target.__data__.text;
...
var wordcloud = function wordcloud() {
  return {
    ...
    Height: 0,
    fill: null,
    go: function go(words, layout, element, selectValuesFunc) {
      var max = Math.max(layout.qHyperCube.qMeasureInfo[0].qMax, 1); // log scale should never cross zero

      var min = Math.max(layout.qHyperCube.qMeasureInfo[0].qMin, 1); // log scale should never cross zero

      var scaleFunction = layout.Scale === "log" ? d3.scaleLog : d3.scaleLinear;
      var scale = scaleFunction().domain([min, max]).rangeRound([layout.MinSize, layout.MaxSize]);
      var from = Math.max(-90, Math.min(90, +layout.RadStart));
      var to = Math.max(-90, Math.min(90, +layout.RadEnd));
      var scaleRotate = d3.scaleLinear().domain([0, +layout.Orientations - 1]).range([from, to]); // Input [0,1] convert into output [-90,90]

      var r = new Random(Random.engines.mt19937().seed(RANDOM_SEED + words.length));
      var self = this;
      return new Promise(function (resolve) {
        d3Cloud().size([self.Width, self.Height]).words(words).padding(5).timeInterval(10).random(function () {
          return r.real(0, 1);
        }).rotate(function () {
          return scaleRotate(Math.round(r.real(0, 1) * (layout.Orientations - 1)));
        }).fontSize(function (d) {
          return scale(+d.value);
        }).font(["QlikView Sans"]).on("end", function (words) {
          return draw(words, layout, element, selectValuesFunc, layout.ScaleColor, self.Id, self.Width, self.Height, resolve);
        }).start();
      });
    },
    id: function id(x) {
...
function paint(element, layout, component) {
  ...
  element.textContent = "";
  $("<div />").attr("id", id).width(size.width).height(size.height).appendTo(element);
  ...
  var cloud = wordcloud().id(id).width(size.width).height(size.height);
...
function supernova() {
  return {
    ...
    component: function component() {
      ...
      var selections = useSelections();
      usePromise(function () {
        var component = {
          selectValues: function selectValues(dim, values, toggle) {
            selections.select({
